...
ipcMain.on('chrome-i18n-getAcceptLanguages', function (evt, responseId) {
  if (evt.sender.isDestroyed()) {
    return
  }
  var response = ['en-US', 'en', 'foo']
  evt.send.send('chrome-i18n-getAcceptLanguages-response-' + responseId, response)
})
